...
;
  ...
  \node [on chain=3,obs, right=3.8cm of y_g] (t_0) {$\hat{t}_0$};
  \node [on chain=3,obs] (t_1) {$\hat{t}_1$};
  \node [on chain=3,obs] (t_2) {$\hat{t}_{2}$};
  \node [on chain=3,obs] {...};
  \node [on chain=3,obs] (t_m) {$\hat{t}_m$};
  ...
  \draw [arrow] (sum.east) to (t_0.west);
  \draw [arrow] (t_m.east) |- (L.west);
